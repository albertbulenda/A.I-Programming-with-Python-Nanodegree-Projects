# PROGRAMMER: BULENDA MWIMANIRE Albert                                                  
# DATE CREATED: 25/10/2021    
# PURPOSE: Development of an image classifer using PyTorch and pre-trained CNN.
...
def validation():
    print("validation parameters process")
    if (args.gpu and not torch.cuda.is_available()):
        raise Exception("--gpu option enabled... Oh oh, no GPU detected")
    if(not os.path.isdir(args.data_directory)):
        raise Exception('no file directory detected')
...
def process_data(data_dir):
    ...
    # definig criteria for Data batching
...
def train(model,data):
    print("Traing process, this might take a few time, please wait")
